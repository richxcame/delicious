...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: this.$tc('action', 2), value: 'actions', align: 'end', sortable: false },
      ],
      items: [],
      search: '',
      alert: '',
  ...
  methods: {
    ...
    setCategory() {
      this.loading = true;
      this.$axios.post('/superadmin/categories', this.category)
        .then(res => {
    ...
    deleteCategory(category) {
      this.$axios.delete(`/superadmin/categories/${category.id}`)
        .then(() => {
